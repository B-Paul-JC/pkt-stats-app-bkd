...
def main():
    # --- ARGUMENT PARSING ---
    parser = argparse.ArgumentParser(description="Generate PDF report from JSON data file.")
    parser.add_argument("input_file", help="Path to the JSON data file")
    parser.add_argument("--output", "-o", default="report.pdf", help="Output PDF filename (default: report.pdf)")
    ...
    try:
        ...
        print(f"Generating PDF for: {meta.get('school_name', 'Unknown Organization')}")
        pdf = CombinedPDF(meta)
        pdf.alias_nb_pages()
        
        count = len(items)
        for i, item in enumerate(items):
            type_ = item.get('type', 'table')
            print(f"Processing item {i+1}/{count}: {type_}")
            
            if type_ == 'table' or (item.get('type') is None and 'data' in item):
